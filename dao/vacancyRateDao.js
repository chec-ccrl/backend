  ...
  getAll: async (vacancyObj, transaction) => {
    try {
      ...
      let sql = `SELECT id , count(*) over() as "total_count" from "vacancyRates" 
                where (province ilike :province or cma ilike :province or ca ilike :province)  and "deletedAt" is null order by "createdAt" desc , "vacancy_rate" desc limit :limit offset :offset`;
